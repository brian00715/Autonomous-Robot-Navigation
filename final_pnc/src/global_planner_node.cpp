...
#include <final_pnc/global_planner_node.h>

void gridmap2charmap(nav_msgs::OccupancyGrid &gridmap, unsigned char *charmap)
{
    for (int i = 0; i < gridmap.data.size(); i++)
    {
        if (gridmap.data[i] == 0)
            charmap[i] = 0;
        else if (gridmap.data[i] == 100)
            charmap[i] = 255;
        else
            charmap[i] = 128;
    }
 ...
 */
GlobalPlanner::GlobalPlanner(ros::NodeHandle *nh, std::string name) : nh_(nh)
{
    ...
    nh_->param("costmap/global_frame", frame_id_, std::string("map"));
    nh_->param("costmap/width", nx_, 20);
    nh_->param("costmap/height", ny_, 20);
    nh_->param("costmap/resolution", resolution_, 0.5);
    nh_->param("costmap/origin_x", origin_x_, 0.0f);
    nh_->param("costmap/origin_y", origin_y_, 0.0f);
    ROS_INFO("frame_id: %s", frame_id_.c_str());
    ROS_INFO("width: %d", nx_);
    ROS_INFO("height: %d", ny_);
    ROS_INFO("resolution: %f", resolution_);
    ROS_INFO("origin_x: %f", origin_x_);
    ROS_INFO("origin_y: %f", origin_y_);

    nh_->param("convert_offset", convert_offset_, 0.0); // offset of transform from world(x,y) to grid map(x,y)
    nh_->param("default_tolerance", tolerance_, 0.0);   // error tolerance
    nh_->param("outline_map", is_outline_, false);      // whether outline the map or not
    nh_->param("obstacle_factor", factor_, 1.0);        // obstacle factor, NOTE: no use...
    nh_->param("expand_zone", is_expand_, false);       // whether publish expand zone or not
    nh_->param("voronoi_map", is_voronoi_map_, false);  // whether to store Voronoi map or not

    // planner name
    nh_->param("planner_name", planner_name_, (std::string) "a_star");
    ...
    else if (planner_name_ == "hybrid_a_star")
    {
        ...
        double dubin_shot_dist; // dubin shot distance
        nh_->param("is_reverse", is_reverse, false);
        nh_->param("max_curv", max_curv, 1.0d);
        nh_->param("dubin_shot_dist", dubin_shot_dist, 3.0d);
        g_planner_ = new global_planner::HybridAStar(nx_, ny_, resolution_, is_reverse, max_curv, dubin_shot_dist);
    }
    else
    ...
    expand_pub_ = nh_->advertise<nav_msgs::OccupancyGrid>("expand", 1);
    make_plan_srv_ = nh_->advertiseService("make_plan", &GlobalPlanner::makePlanService, this);
    gridmap_pub_ = nh_->advertise<nav_msgs::OccupancyGrid>("costmap", 10, true);

    // subs
    gridmap_sub_ = nh_->subscribe<nav_msgs::OccupancyGrid>("/move_base/global_costmap/costmap", 10, &GlobalPlanner::gridmapCallback, this);
    gridmap_update_sub_ = nh_->subscribe<map_msgs::OccupancyGridUpdate>("/move_base/global_costmap/costmap_updates", 10, &GlobalPlanner::gridmapUpdateCallback, this);
}
                             ...
                             double tolerance, std::vector<geometry_msgs::PoseStamped> &plan)
{
    ...
    // outline the map
    if (is_outline_)
        g_planner_->outlineMap(charmap_);
    ...
    else if (planner_name_ == "hybrid_a_star")
    {
        ...
        path_found = dynamic_cast<global_planner::HybridAStar *>(g_planner_)->plan(charmap_, h_start, h_goal, path, expand);
    }
    else
        path_found = g_planner_->plan(charmap_, start_node, goal_node, path, expand);
 ...
 */
void GlobalPlanner::_publishExpand(std::vector<Node> &expand)
{
    ...
    double wx, wy;
    g_planner_->map2World(0, 0, wx, wy);
 ...
 */
bool GlobalPlanner::_getPlanFromPath(std::vector<Node> &path, std::vector<geometry_msgs::PoseStamped> &plan)
{
    if (!initialized_)
    {
        ROS_ERROR("This planner has not been initialized yet, but it is being used, please call initialize() before use");
...
{
    gridmap_ = *msg;
    charmap_ = new unsigned char[gridmap_.data.size()];
    ROS_INFO("Received costmap with size: %d x %d", msg->info.width, msg->info.height);
}

void GlobalPlanner::gridmapUpdateCallback(const map_msgs::OccupancyGridUpdate::ConstPtr &msg)
{
    int index = 0;
    for (int y = msg->y; y < msg->y + msg->height; y++)
    {
        ...
        {
            gridmap_.data[g_planner_->grid2Index(x, y)] = msg->data[index++];
        }
    }
    gridmap2charmap(gridmap_, charmap_);
...
int main(int argc, char **argv)
{
    ...
    GlobalPlanner planner(&nh, "global_planner");
