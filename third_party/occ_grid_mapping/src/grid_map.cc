...
GridMap::GridMap(const nav_msgs::OccupancyGrid& occ_grid)
{
    size_x_ = occ_grid.info.width;
    size_y_ = occ_grid.info.height;
    cell_size_ = occ_grid.info.resolution;
    init_x_ = -occ_grid.info.origin.position.x / cell_size_;
    init_y_ = -occ_grid.info.origin.position.y / cell_size_;
    // std::cout << "size_x: " << size_x_ << " size_y: " << size_y_ << " init_x: " << init_x_ << " init_y: " << init_y_ << " cell_size: " << cell_size_ << std::endl;
    bel_data_.resize(size_x_, size_y_);
    // std::cout << "size_x: " << size_x_ << " size_y: " << size_y_ << std::endl;
    ...
    for(size_t i = 0; i < N; i++)
    {
        ...
        if(value == -1)
            bel_data_.data()[i] = 0.5;
        else if (value < 45)
            bel_data_.data()[i] = 0.0;
        else if (value > 55)
            bel_data_.data()[i] = 1.0;
        else
            bel_data_.data()[i] = 0.5;
    }
